...
function createWindow() {
  var opt = {
    ...
    minWidth: 1020,
    minHeight: 660,
    title: custom.title || "WOS Browser",
  ...
  win.setTitle(custom.title || "WOS Browser");
...
ipcMain.on('asynchronous', (event, data) => {
  switch (data.key) {
    ...
    case 'installRestart':
      ...
      //Copy
      //var from = path.join(os.homedir(), '.wos-browser', version+'-app.asar');
      ...
      moveFile(from, to, function(e){
        if(e)fs.writeFileSync(path.join(os.homedir(), '.wos-browser','upgrade-error.txt'), JSON.stringify(e));
        app.relaunch();
        app.exit(0);
      });
