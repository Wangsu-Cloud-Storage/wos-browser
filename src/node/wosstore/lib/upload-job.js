...
class UploadJob extends Base {
   ...
   *    config.to   {object|string}  {bucket, key} or wos://bucket/test/a.jpg
  ...
  constructor(wosClient, config) {
    ...
    this.to = util.parseWosPath(this._config.to);
...
UploadJob.prototype.start = function () {
  ...
  if(isLog == 1 && isLogInfo == 1) {
    ...
    log.info(`[${self.to.bucket}] ----start upload ${self.from.path} as  ${self.to.key}` );
  }
...
UploadJob.prototype.stop = function () {
  ...
  if(isLog == 1 && isLogInfo == 1) {
    log.transports.file.level = 'info';
    log.info(`-----stop upload ${this.from.path}`);
  }
...
UploadJob.prototype.wait = function () {
  ...
  if(isLog == 1 && isLogInfo == 1) {
    log.transports.file.level = 'info';
    log.info(`-----wait upload ${this.from.path}`);
  }
...
//crc 校验失败，删除wos文件
UploadJob.prototype.deleteWosFile = function(){
   var self = this;
   self.wos.deleteObject({Bucket: self.to.bucket, Key: self.to.key}, function(err){
     if(err) console.error(err);
     else console.log('crc64 verifying failed, wos file [wos://'+  self.to.bucket+'/'+self.to.key+'] is deleted');
   });
};
...
UploadJob.prototype.startUpload = function () {
  ...
  if(isLog == 1 && isLogInfo == 1) {
    log.transports.file.level = 'info';
    log.info(`[${self.to.bucket}] ----prepareChunks ${self.from.path} for  ${self.to.key}`);
  }
  ...
  util.prepareChunks(self.from.path, self.checkPoints, function (err, checkPoints) {
    ...
    if (checkPoints.chunks.length == 1 && checkPoints.chunks[0].start==0) {
      ...
      if(isLog == 1 && isLogInfo == 1) {
        log.transports.file.level = 'info';
        log.info(`[${self.to.bucket}] ----uploadSingle ${self.from.path} as  ${self.to.key}`);
      }
      ...
      if(isLog == 1 && isLogInfo == 1) {
        log.transports.file.level = 'info';
        log.info(`[${self.to.bucket}] ----uploadMultipart ${self.from.path} as  ${self.to.key}`);
      }
...
UploadJob.prototype.uploadSingle = function () {
  ...
  self.prog = {
      loaded: 0,
      total: stat.size // Buffer.byteLength(data)
  };

  var retryTimes = 0;
  _dig();
  function _dig(){
      var req = self.wos.putObject(params, function (err,data) {
          ...
          if (err) {
              ...
              util.checkFileHash(self.from.path, data['HashCrc64ecma'], data['ContentMD5'], function(err){
                  if(err){
                      ...
                      self.emit('complete');
                      if (isLog == 1 && isLogInfo == 1) {
                        log.info(`[${self.to.bucket}] ----upload ${self.from.path} as  ${self.to.key} succeeded!`)
                      }
...
UploadJob.prototype.uploadMultipart = function (checkPoints) {
  ...
  function doUpload(partParams){
    // 解决实际上传成功但点击暂停又开始后一直重试的问题
    if(self.multipartComplete){
      console.log('上传块完成');
    ...
    var req = self.wos.uploadPart(partParams, function (multiErr, mData) {
