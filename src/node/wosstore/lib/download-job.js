...
class DownloadJob extends Base {
   ...
   * @param wosClient
   * @param config
   *    config.from {object|string}  {bucket, key} or wos://bucket/test/a.jpg
  ...
  constructor(wosClient, config) {
    ...
    this.from = util.parseWosPath(this._config.from); //wos path
...
DownloadJob.prototype.startDownload = function (checkPoints) {
  ...
  util.headObject(self, objOpt, function (err, headers) {
    ...
    || err.message.indexOf('getaddrinfo ENOTFOUND')!=-1){
        self.message = 'failed to get wos object meta: ' + err.message;
  ...
  function downloadPart(n) {
    ...
    function doDownload(n) {
      ...
      var req = self.wos.getObject(obj, (err, data) => {
